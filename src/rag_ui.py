...
data_path = os.path.join("src/", "huberman_labs.json")
model_path = os.path.join("src/", "allminilm-finetuned-256")
...
## Display properties
display_properties = [
    prop.name for prop in retriever.show_collection_properties(collection_name)
]
...
available_collections = [
    "Huberman_allminilm_256"
]  # the rest of the collections were deleted
...
def main(retriever: WeaviateWCS):
    ...
    with st.sidebar:
        ...
        alpha_input = st.slider(
            "Alpha", min_value=0.0, max_value=1.0, value=0.5, step=0.01
        )
        retrieval_limit = st.slider(
            "Retrieval Limit", min_value=1, max_value=200, value=10, step=1
        )
        reranker_topk = st.slider(
            "Reranker Topk", min_value=1, max_value=10, value=3, step=1
        )
        ...
        with st.spinner("Generating Response..."):
            ...
            if make_llm_call:
                with st.chat_message(
                    "Huberman Labs", avatar="src/app_assets/huberman_logo.png"
                ):
